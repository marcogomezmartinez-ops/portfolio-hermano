...
# ── Invesco Physical Silver vía Yahoo Finance (SSLV.L — GBP → EUR) ──────────
try:
    r_gbpusd = requests.get('https://query1.finance.yahoo.com/v8/finance/chart/GBPUSD=X?interval=1d&range=1d',
                     headers={'User-Agent': 'Mozilla/5.0'}, timeout=15)
    gbp_usd = r_gbpusd.json()['chart']['result'][0]['meta']['regularMarketPrice']
    r_silver = requests.get('https://query1.finance.yahoo.com/v8/finance/chart/SSLV.L?interval=1d&range=1d',
                     headers={'User-Agent': 'Mozilla/5.0'}, timeout=15)
    price_silver_gbp = r_silver.json()['chart']['result'][0]['meta']['regularMarketPrice']
    eur_usd = prices.get('EURUSD', 1.12)
    price_silver_eur = round(price_silver_gbp * gbp_usd / eur_usd, 5)
    prices['IE00B43VDT70'] = price_silver_eur
    print(f"  ✅ Invesco Physical Silver: £{price_silver_gbp} → €{price_silver_eur}")
except Exception as e:
