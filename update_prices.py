...
# ── Invesco Physical Silver vía XAG/USD spot (0.9319 oz/título × EUR/USD) ────
try:
    r_xag = requests.get('https://query2.finance.yahoo.com/v8/finance/chart/XAGUSD=X?interval=1d&range=5d',
                     headers={'User-Agent': 'Mozilla/5.0'}, timeout=15)
    xag_usd = r_xag.json()['chart']['result'][0]['meta']['regularMarketPrice']
    eur_usd = prices.get('EURUSD', 1.12)
    OZ_PER_SHARE = 0.9319  # Invesco Physical Silver IE00B43VDT70 (calibrado 15/05/2026)
    price_silver_eur = round(xag_usd * OZ_PER_SHARE / eur_usd, 5)
    prices['IE00B43VDT70'] = price_silver_eur
    print(f"  ✅ Invesco Physical Silver: ${xag_usd}/oz × {OZ_PER_SHARE} → €{price_silver_eur}")
except Exception as e:
