...
# iShares Physical Gold via Yahoo Finance (IGLN.L en GBp → convertir a EUR)
try:
    r = requests.get('https://query1.finance.yahoo.com/v8/finance/chart/IGLN.L?interval=1d&range=1d',
                     headers={'User-Agent': 'Mozilla/5.0'}, timeout=15)
    price_gbp = r.json()['chart']['result'][0]['meta']['regularMarketPrice'] / 100  # GBp a GBP
    fx = requests.get('https://query1.finance.yahoo.com/v8/finance/chart/GBPEUR=X?interval=1d&range=1d',
                      headers={'User-Agent': 'Mozilla/5.0'}, timeout=15)
    gbpeur = fx.json()['chart']['result'][0]['meta']['regularMarketPrice']
    price_eur = round(price_gbp * gbpeur, 5)
